...
import pandas as pd
import numpy as np
from data_loader.data_loader_attnmove import tdrive_data_loader
...
for epoch in range(train_epochs):
    ...
    for i, (hist_traj_grid, userId, mask_pos, mask_curr_traj_grid, mask_value, curr_traj_grid,
        ...
        # with torch.no_grad():
        #     value, index = torch.topk(outputs, 1, dim=1)
        #     index = index.cpu().detach().numpy().reshape(-1, 10)
        #     true_index = (mask_value - 3).cpu().detach().numpy().reshape(-1, 10)
        #
        #     true_count = 0
        #     false_count = 0
        #     for i in range(len(index)):
        #         if true_index[i] in index[i]:
        #             true_count += 1
        #         else:
        #             false_count += 1
        #     print(true_count, false_count, round(true_count / (true_count + false_count), 4))

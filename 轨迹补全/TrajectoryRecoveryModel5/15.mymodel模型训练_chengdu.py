...
import pandas as pd
import numpy as np
from data_loader.data_loader_mymodel import chengdu_data_loader
...
for i in range(4,4+dm):
    ...
    if i == 4:
        val_loader = DataLoader(val_set, batch_size=batch_size * 4, shuffle=True, num_workers=0, drop_last=False)
        test_loader = DataLoader(test_set, batch_size=batch_size * 4, shuffle=True, num_workers=0, drop_last=False)
...
# 3.训练模型
train_epochs = 2000
model_optim = optim.Adam(model.parameters(), lr=0.0021, weight_decay=0.00)
        ...
        for j, (val_hist_traj_grid, val_userId, val_mask_pos, val_mask_curr_traj_grid, val_mask_value,
            ...
            if round(true_count / (fasle_count + true_count), 4) > max_val:
                ...
                torch.save(model.state_dict(),
                           'pars/mymodel/mymodel_chengdu_recall@{}_{}_dm'.format(recall,

...
initial_file_path = './res/ATDC/data1.txt'
...
next_time = time.time()
times.append(round(next_time - time_start, 2))
print('初始DBA轨迹生成结束')
...
threshold = 3.5
count_normal = 20
test_traj_index = random.sample(range(0, len(traj_list)), len(traj_list))
for index in test_traj_index:
    if index not in normal_file_index:
        curr_traj = traj_list[index]
        cur_dtw, _ = fastdtw(DBA_list, curr_traj, dist=lambda x, y: abs(x[0] - y[0]) + abs(x[1] - y[1]))
...
open('./res/time/A_1_dtw.txt', mode='a').writelines(' '.join(times) + '\n')

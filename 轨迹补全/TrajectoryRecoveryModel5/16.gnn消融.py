...
import pandas as pd
import numpy as np
from data_loader.data_loader_mymodel import baxi_data_loader,tdrive_data_loader,shenzhen_data_loader,chengdu_data_loader
from model.mymodel_ab_gnn import MyModel
...
for i in range(4,4+dm):
    np.random.seed(i + 1)
    # baxi_data_loader  baxi/baxi_bus_10_500_43.16_43.7226_22.7858_23.0668(4).csv
    # tdrive_data_loader t_drive/t_drive_dispersed_500_116.00211_116.79979_39.5018_40.19988.csv
    # shenzhen_data_loader shenzhen_taxi/shenzhen_texi_10_500_113.6139_114.5961_22.461_22.9998.csv
    # chengdu_data_loader  chengdu_taxi/chengdu_interval_10_500_103.2697_104.6097_30.2907_31.0325(1).csv
    temp_set = chengdu_data_loader(
        path='processed_data/chengdu_taxi/chengdu_interval_10_500_103.2697_104.6097_30.2907_31.0325(1).csv', mask=mask, time_step=time_step)
    ...
    if i == 4:
        ...
        test_loader = DataLoader(test_set, batch_size=256 * 3, shuffle=True, num_workers=0, drop_last=False)
...
# 3.训练模型
train_epochs = 2000
model_optim = optim.Adam(model.parameters(), lr=0.002, weight_decay=0.01)
...
for epoch in range(train_epochs):
    ...
    for i, (hist_traj_grid, userId, mask_pos, mask_curr_traj_grid, mask_value, curr_traj_grid, dtw_border,_,__) in enumerate(
    ...
    with torch.no_grad():
        for j, (val_hist_traj_grid, val_userId, val_mask_pos, val_mask_curr_traj_grid, val_mask_value,
            ...
            if round(true_count / (fasle_count + true_count), 4) > max_val:
                ...
                torch.save(model.state_dict(),
                           'pars/mymodel/mymodel_baxi_recall@{}_{}_dm'.format(recall,
        ...
        for j, (test_hist_traj_grid, test_userId, test_mask_pos, test_mask_curr_traj_grid, test_mask_value,
                test_curr_traj_grid, test_dtw_border,_,__) in enumerate(test_loader):
            ...
            for k in range(len(index)):
                ...
                if true_index[k] in index[k]:
                    ...
                    fasle_count += 1
            print('测试集:正确的位置个数{},错误的位置个数{},正确率{}'.format(true_count, fasle_count,
                                                         round(true_count / (fasle_count + true_count), 4)))
    schedule.step()
    print(

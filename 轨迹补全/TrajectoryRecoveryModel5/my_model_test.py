import sys
sys.path.insert(0,'/content/drive/MyDrive/MyFile')
import torch
from torch.utils.data import DataLoader
import pandas as pd
import numpy as np
from data_loader.data_loader_mymodel import shenzhen_data_loader, baxi_data_loader, construct_graph, tdrive_data_loader
...
if __name__ == '__main__':
    ...
    # baxi/baxi_bus_10_500_43.16_43.7226_22.7858_23.0668(4)
    ...
    model = MyModel(d_model=d_model, heads=heads, grid_count=t.grid_count, user_count=t.user_count,
                    time_step=time_step, step=step)

    # mymodel_tdrive_recall@1_2022-01-13 17_25_49
    # mymodel_tdrive_recall@1_2022-05-30 03_49_51_dm
    # 3.加载模型
    model.load_state_dict(
        torch.load('pars/mymodel/mymodel_chengdu_recall@1_2022-06-05 06_11_30_dm', map_location=torch.device('cpu')))
    max_val = 0
    with torch.no_grad():
        for j, (test_hist_traj_grid, test_userId, test_mask_pos, test_mask_curr_traj_grid, test_mask_value,
            ...
            recovery_df.to_csv('res/chengdu/mymodel_chengdu_error.csv', index=False)
            if true_count != 0:
